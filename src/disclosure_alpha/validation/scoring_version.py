...
def normalize_scoring_version(version: str) -> str:
    ...
    if key in ("v1", "1", SCORING_MODEL_VERSION_V1):
        return SCORING_MODEL_VERSION_V1
    if key in ("v2", "2", SCORING_MODEL_VERSION, SCORING_MODEL_VERSION_V2):
        return SCORING_MODEL_VERSION
    raise ValueError(f"unsupported scoring version: {version!r} (use v1 or v2)")


def is_v1_scoring(version: str) -> bool:
    return normalize_scoring_version(version) == SCORING_MODEL_VERSION_V1
